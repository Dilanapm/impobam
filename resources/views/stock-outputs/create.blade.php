...
<body class="bg-canvas text-foreground">
    <div class="mx-auto max-w-5xl p-3 sm:p-4">
        ...
        <div class="rounded-3xl bg-surface p-4 shadow-lg sm:p-6 md:p-8">
            ...
            <div class="mt-5 rounded-2xl border border-primary-border bg-primary-soft p-4 sm:p-5">
                ...
                <ol class="mt-4 space-y-3 text-lg leading-relaxed text-foreground sm:text-xl">
                ...
                @forelse($recentOutputs as $output)
                    <div class="rounded-2xl border border-border bg-muted p-4 shadow-sm sm:p-5">
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                <span class="mb-1 block text-sm font-bold uppercase tracking-wide text-foreground-muted">📅
                                    Fecha</span>
                ...
                <form method="POST" action="{{ route('stock-outputs.store') }}" class="mt-6 space-y-6"
                    ...
                    <div>
                        ...
                        <select name="product_id" id="product_id" required
                            class="min-h-[64px] w-full rounded-2xl border-2 border-border-strong bg-surface px-4 py-3 text-lg text-foreground shadow-sm outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/20 sm:text-xl">
                    ...
                    <div>
                        <label for="employee_name"
                            class="mb-3 flex items-center gap-2 text-xl font-bold text-foreground sm:text-2xl">
                            <span class="text-2xl">👤</span>
                            <span>Nombre del empleado <span class="font-normal text-foreground-muted">(opcional)</span></span>
                        </label>
                    ...
                    <div>
                        <label for="notes"
                            class="mb-3 flex items-center gap-2 text-xl font-bold text-foreground sm:text-2xl">
                            <span class="text-2xl">📝</span>
                            <span>Observación <span class="font-normal text-foreground-muted">(opcional)</span></span>
                        </label>
                        <textarea name="notes" id="notes" placeholder="Detalle adicional del retiro"
                            class="min-h-[140px] w-full rounded-2xl border-2 border-border-strong bg-surface px-4 py-3 text-lg text-foreground shadow-sm outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/20 sm:text-xl">{{ old('notes') }}</textarea>
                        @error('notes')
                            <div class="mt-2 text-lg font-medium text-danger" data-validation-error>⚠️ {{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <button type="button" id="previewButton"
                        class="inline-flex min-h-[64px] w-full items-center justify-center gap-3 rounded-2xl bg-primary px-6 py-4 text-xl font-bold text-primary-foreground transition hover:bg-primary-hover active:scale-[0.99] sm:text-2xl">
                        <span class="text-2xl">👀</span>
        ...
        <x-confirm-modal modal-id="previewModal" cancel-id="cancelPreview" confirm-id="confirmSubmit"
            ...
            <div>
                ...
                <p id="previewProduct" class="text-xl font-semibold text-foreground sm:text-2xl">—</p>
            </div>

            <div>
                <span class="mb-1 block text-sm font-bold uppercase tracking-wide text-foreground-muted">🔢 Cantidad</span>
                <p id="previewQuantity" class="text-xl font-semibold text-foreground sm:text-2xl">—</p>
            </div>

            <div>
                <span class="mb-1 block text-sm font-bold uppercase tracking-wide text-foreground-muted">👤 Empleado</span>
                <p id="previewEmployee" class="text-xl font-semibold text-foreground sm:text-2xl">—</p>
            </div>

            <div>
                <span class="mb-1 block text-sm font-bold uppercase tracking-wide text-foreground-muted">📝 Observación</span>
                <p id="previewNotes" class="text-xl font-semibold text-foreground sm:text-2xl">—</p>
            </div>

            <div>
                <span class="mb-1 block text-sm font-bold uppercase tracking-wide text-foreground-muted">🕒 Se guardará
                    con</span>
                <p id="previewDateTime" class="text-xl font-semibold text-foreground sm:text-2xl">—</p>
            </div>
        </x-confirm-modal>
